...
model_path = os.path.join(SHARED_AIMODELS, 'estrus_model_v1.pth')

# 디렉토리가 없으면 생성
os.makedirs(os.path.dirname(model_path), exist_ok=True)

# 3️⃣ 모델 저장 (.pth 파일로 저장)
#torch.save(model, model_path) # 모델 전체객체 저장
#print(f"Model saved at: {model_path}")
torch.save(model.state_dict(), model_path) # 모델 가중치만 저장
...
torch.save()를 사용하여 모델을 "backend/services_ai/estrus_detection/aimodels/estrus_model.pth" 경로에 저장합니다."""
...
python
torch.save(model, "backend/services_ai/estrus_detection/aimodels/estrus_model.pth")
